	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (
				...
				return toast.error(
					"Por favor, completa todos los campos obligatorios antes de crear la rifa."
				)
			} else {
			...
			setSuccess(true)
			if (createdRaffle?.id) {
				navigate(`/raffles/${createdRaffle.id}`)
			} else {
				setError("Rifa creada pero no se pudo redirigir correctamente.")
			}
		} catch (err) {
